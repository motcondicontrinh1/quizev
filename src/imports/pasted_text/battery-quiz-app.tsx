...
import { Play, ArrowRight, ArrowLeft, CheckCircle2, XCircle, RotateCcw, Terminal, Zap, Menu, X } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0f0f0f] text-white flex flex-col items-center justify-center font-mono">
        ...
        <p className="text-[14px] text-white/50 uppercase tracking-widest">Đang thiết lập trạm...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#0f0f0f] text-white relative overflow-x-hidden selection:bg-[#00ffff] selection:text-black">
      <style dangerouslySetInnerHTML={{ __html: customStyles }} />
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-12 md:py-20 relative z-10 min-h-screen flex flex-col">
        {/* Navigation Bar */}
        <header className="flex items-center justify-between border-b border-white/[0.08] pb-6 mb-12">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-black border border-white/[0.12] rounded flex items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,255,255,0.15)]">
              <Zap className="w-4 h-4 text-[#00ffff]" />
            </div>
            <span className="font-mono text-[14px] uppercase tracking-[0.5px] font-bold text-white/90 hidden sm:inline">COMPOSIO_ASSESS</span>
          </div>
          {screen === 'quiz' && (
        ...
        {/* --- START SCREEN --- */}
        {screen === 'start' && (
          <div className="flex-1 flex flex-col items-center justify-center text-center max-w-[800px] mx-auto w-full mt-[-8vh]">
            <span className="font-mono text-[12px] uppercase tracking-[1px] text-[#00ffff] bg-[#00ffff]/10 px-3 py-1 border border-[#00ffff]/20 rounded-sm mb-8 inline-flex">
              Version 1.0.4 • System Ready
            </span>
            <h1 className="text-[56px] md:text-[80px] font-sans font-normal leading-[0.87] tracking-[-0.03em] mb-8 drop-shadow-lg">
              Trạm Đánh Giá <br/> Năng Lực Cốt Lõi
            </h1>
            <p className="text-[16px] md:text-[18px] text-white/60 mb-12 leading-[1.6] max-w-[600px] font-sans">
              Hệ thống kiểm tra trình độ chuyên viên lắp ráp cell pin. 
              Mỗi module yêu cầu sự chính xác tuyệt đối. Dữ liệu sẽ được khóa sau mỗi quyết định.
            </p>
            <button 
              onClick={handleStart}
              className="group relative bg-white text-[#111] px-8 py-4 text-[16px] font-sans font-medium hover:bg-white/90 transition-all flex items-center gap-3 active:scale-[0.98]"
            >
              BẮT ĐẦU KIỂM TRA
        ...
        {/* --- QUIZ SCREEN --- */}
        {screen === 'quiz' && (
          <div className="flex-1 flex flex-col max-w-[800px] mx-auto w-full pb-28 md:pb-24">
            {/* Question Area */}
            <div className="bg-[#000000] border border-white/[0.10] rounded-[4px] p-6 md:p-10 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] flex-1 flex flex-col relative z-10">
                
                {/* Accent line */}
                <div className="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-[#0007cd] via-[#00ffff] to-transparent opacity-50" />

                <div className="mb-8">
                  <span className="font-mono text-[12px] uppercase tracking-[0.5px] text-white/40 block mb-4">
                    [ RECORD_{String(currentIndex + 1).padStart(3, '0')} ]
                  </span>
                  <h2 className="text-[20px] md:text-[28px] font-sans leading-[1.4] text-white">
                    {currentQ.q}
                  </h2>
                </div>
                ...
                <div className="space-y-3 flex-1">
                  {currentQ.options.map((opt, idx) => {
                    ...
                    let optionClasses = "w-full text-left p-4 md:p-5 rounded-[2px] border font-sans text-[15px] md:text-[16px] leading-[1.5] transition-all relative overflow-hidden ";
                    
                    if (!isAnswered) {
                      optionClasses += "bg-transparent border-white/[0.10] text-white/70 hover:border-[#0089ff] hover:text-white hover:bg-white/[0.02]";
                    } else {
                      optionClasses += "cursor-default ";
                      if (isSelected && isCorrect) {
                        optionClasses += "border-[#00ffcc] bg-[#00ffcc]/[0.08] text-[#00ffcc] shadow-[0_0_15px_rgba(0,255,204,0.1)]";
                      } else if (isSelected && !isCorrect) {
                        optionClasses += "border-[#ff003c] bg-[#ff003c]/[0.08] text-[#ff003c]";
                      } else if (!isSelected && isCorrect) {
                        optionClasses += "border-[#00ffcc]/40 text-[#00ffcc]/80 bg-transparent";
                      } else {
                        optionClasses += "border-white/[0.04] text-white/20 bg-transparent";
                      }
                    }
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-4">
                          <span className="font-mono text-[14px] mt-0.5 opacity-60">
                            {['01', '02', '03', '04'][idx]}
                          </span>
                          <span className="flex-1">{opt}</span>
                          
                          {/* Icons validation */}
                          {isAnswered && isSelected && isCorrect && <CheckCircle2 className="w-5 h-5 text-[#00ffcc] shrink-0 mt-0.5" />}
                          {isAnswered && isSelected && !isCorrect && <XCircle className="w-5 h-5 text-[#ff003c] shrink-0 mt-0.5" />}
                        </div>
                      </button>
                    );
                  })}
                ...
                {answers[currentIndex] && (
                  <div className="mt-8 border border-[#0089ff]/30 bg-[#0089ff]/[0.03] p-5 rounded-[2px] animate-fade-in">
                    <div className="flex items-center gap-2 mb-2 font-mono text-[12px] text-[#0089ff] uppercase">
                      <Terminal className="w-3 h-3" /> System_Output / Lý giải
                    </div>
                    <p className="font-sans text-[14px] md:text-[15px] text-white/80 leading-[1.6]">
                      {currentQ.explain}
                    </p>
                  </div>
                )}
            ...
            <div className="fixed bottom-0 left-0 w-full bg-[#0f0f0f]/90 backdrop-blur-md border-t border-white/[0.08] p-4 md:p-6 z-40 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
              <div className="max-w-[800px] mx-auto flex items-center justify-between gap-3 md:gap-6">
                ...
                >
                  ...
                  {currentIndex < questions.length - 1 ? <ArrowRight className="w-5 h-5 md:w-4 md:h-4" /> : <CheckCircle2 className="w-5 h-5 md:w-4 md:h-4" />}
                </button>
              </div>
            </div>
            ...
            {showGrid && (
              ...
              >
                <div className="w-full max-w-[800px] bg-[#000000] border-t border-l border-r border-white/[0.12] rounded-t-[12px] p-6 pb-[calc(24px+env(safe-area-inset-bottom))] animate-slide-up shadow-[0_-8px_40px_rgba(0,0,0,0.8)] relative">
                  
                  {/* Handle for drag indicator aesthetic */}
                  <div className="w-12 h-1.5 bg-white/20 rounded-full mx-auto mb-6 cursor-pointer hover:bg-white/30 transition-colors" onClick={() => setShowGrid(false)} />
                  ...
                  <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/[0.08]">
                    <div>
                      ...
                      <p className="font-sans text-[13px] text-white/40">
                        {Object.keys(answers).length}/{questions.length} MODULE ĐÃ HOÀN THÀNH
                      </p>
                    </div>
                    <button 
                  ...
                  <div className="grid grid-cols-5 sm:grid-cols-8 gap-3 md:gap-4 mb-8">
                    {questions.map((q, idx) => {
                      ...
                      let btnClasses = "aspect-square flex items-center justify-center font-mono text-[14px] md:text-[16px] rounded-[2px] border transition-all relative ";
                      
                      if (!isAnswered) {
                        btnClasses += "border-white/[0.1] text-white/40 hover:border-white/30 hover:text-white/80 ";
                      } else if (isCorrect) {
                        btnClasses += "border-[#00ffcc]/30 bg-[#00ffcc]/10 text-[#00ffcc] shadow-[inset_0_0_10px_rgba(0,255,204,0.05)] ";
                      } else {
                        btnClasses += "border-[#ff003c]/30 bg-[#ff003c]/10 text-[#ff003c] shadow-[inset_0_0_10px_rgba(255,0,60,0.05)] ";
                      }

                      if (isCurrent) {
                        btnClasses += "!border-[#00ffff] !text-[#00ffff] shadow-[0_0_15px_rgba(0,255,255,0.2)] bg-[#00ffff]/5 outline outline-1 outline-offset-2 outline-[#00ffff]/50 ";
                      }
                  ...
                  {/* Legend */}
                  <div className="flex flex-wrap items-center gap-6 font-sans text-[13px] text-white/50 justify-center bg-white/[0.02] p-4 rounded-[4px] border border-white/[0.04]">
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-[2px] border border-white/[0.1] bg-black" /> Chưa làm
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-[2px] border border-[#00ffcc]/30 bg-[#00ffcc]/10" /> Đạt chuẩn
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-[2px] border border-[#ff003c]/30 bg-[#ff003c]/10" /> Cảnh báo lỗi
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* --- RESULT SCREEN --- */}
        {screen === 'result' && (
          <div className="flex-1 flex flex-col items-center justify-center text-center animate-fade-in w-full max-w-[600px] mx-auto mt-[-5vh]">
            <div className="w-full bg-[#000000] border border-white/[0.12] p-10 md:p-14 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] rounded-[4px] relative overflow-hidden">
              {/* Glow overlay */}
              ...
              <p className="font-mono text-[14px] uppercase tracking-[2px] text-white/50 mb-6">ĐÁNH GIÁ HOÀN TẤT</p>
              
              <div className="flex items-baseline justify-center gap-2 mb-4">
                <span className="font-mono text-[80px] md:text-[100px] leading-[1] text-white font-normal tracking-tighter">
                  {score}
                </span>
                <span className="font-mono text-[30px] md:text-[40px] text-white/30">
                  / {questions.length}
                </span>
              </div>
              
              <h2 className="text-[20px] md:text-[24px] font-sans font-medium text-[#00ffff] mb-8">
                {score === questions.length ? "XUẤT SẮC - HỆ THỐNG ĐỒNG BỘ" : 
                 score >= questions.length / 2 ? "ĐẠT YÊU CẦU - CẦN TỐI ƯU THÊM" : 
              ...
              <button 
                onClick={handleRetry}
                className="group w-full md:w-auto mx-auto border border-[#2c2c2c] bg-transparent text-white hover:border-[#0089ff] px-8 py-3 flex items-center justify-center gap-3 transition-colors font-sans text-[15px] uppercase tracking-[0.5px]"
              >
